...
def reproject_with_depth(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    ...
    print('depth_src.shape:',depth_src.shape, x_src.shape, y_src.shape)
    sampled_depth_src = cv2.remap(depth_src, x_src, y_src, interpolation=cv2.INTER_LINEAR)
    '''
    print('depth_src.shape:',depth_src, depth_src.shape, xy_src, xy_src.shape)
    sampled_depth_src = cv2.remap(depth_src,
                              xy_src[0].reshape([height, width]).astype(np.float32),
                              xy_src[1].reshape([height, width]).astype(np.float32),
...
if __name__ == '__main__':
    ...
    for scan in scans:
        ...
        out_folder = os.path.join(args.outdir, 'checkpoints_model_000004.ckpt')
        out_folder = os.path.join(out_folder,  scan)
